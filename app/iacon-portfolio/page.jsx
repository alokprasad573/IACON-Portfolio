...
const CyberScreen = ({ children, accentColor = "#FFD700", glowLevel = 20 }) => {
  return (
    <div
      className="crt-reveal layer-shrink relative w-full max-w-[90%] md:max-w-[1000px] bg-black/90 border backdrop-blur-xl overflow-hidden p-8 md:p-12 transition-all duration-500 accelerated"
      style={{
        clipPath: 'polygon(0% 40px, 40px 0%, calc(100% - 40px) 0%, 100% 40px, 100% calc(100% - 40px), calc(100% - 40px) 100%, 40px 100%, 0% calc(100% - 40px))',
        boxShadow: `0 0 ${glowLevel}px ${accentColor}15`,
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 left-1/4 right-1/4 h-px bg-brand-yellow/40 shadow-[0_0_15px_#FFD700]"></div>
        <div className="absolute bottom-0 left-1/4 right-1/4 h-px bg-brand-yellow/40 shadow-[0_0_15px_#FFD700]"></div>
        <div className="absolute left-0 top-1/4 bottom-1/4 w-px bg-brand-yellow/20"></div>
        <div className="absolute right-0 top-1/4 bottom-1/4 w-px bg-brand-yellow/20"></div>
      </div>
...
const StackLayer = ({ children, index }) => {
  return (
    <div
      className="scroll-layer sticky top-0 h-screen w-full flex items-center justify-center p-4 overflow-hidden bg-bg-deep"
      style={{ zIndex: index }}
    >
...
const App = () => {
  return (
    <div className="bg-[#050505] text-gray-300 font-sans selection:bg-yellow-500 selection:text-black">
      ...
      <StackLayer index={0}>
        <CityScanBackground />
        <section className="hero-contents relative h-screen flex flex-col items-center justify-center text-center px-6 z-10 overflow-hidden">
      ...
      <div className="relative z-20 stack-container">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {Object.entries(techStck).map(([cat, skills]) => (
                <div key={cat} className="bg-black/50 border border-white/10 p-4 backdrop-blur-md hover:border-yellow-500/50 transition-all group">
                  <h3 className="text-yellow-500 font-black uppercase text-[9px] mb-3 tracking-widest group-hover:translate-x-1 transition-transform">{cat}</h3>
                  <div className="flex flex-wrap gap-1">
                    {skills.map(s => <span key={s} className="px-1.5 py-0.5 bg-black text-[8px] border border-white/5 font-mono uppercase text-gray-400">{s}</span>)}
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* PROJECTS */}
        {projects.map((proj, i) => (
          <StackLayer key={i} index={i + 2}>
            <CyberScreen>
              <div className="grid md:grid-cols-2 gap-8 items-center h-full">
                <div className="space-y-6">
                  <div className="text-yellow-500 font-mono text-[9px] tracking-[0.5em] uppercase">Deployment_0{i + 1}</div>
                  <h3 className="text-3xl md:text-5xl font-black text-white italic uppercase leading-none">{proj.title}</h3>
                  <p className="text-gray-400 font-mono text-[11px] leading-relaxed max-w-md">"{proj.description}"</p>
        ...
        {/* ACADEMIC */}
        <StackLayer index={projects.length + 2}>
          <CyberScreen>
            ...
            <h4 className="text-3xl md:text-5xl font-black text-white italic uppercase mb-2">{educationData[0].institute}</h4>
            <p className="text-lg text-yellow-500 font-black uppercase mb-8 border-b border-yellow-500/20 pb-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
              <div>
                ...
                <div className="text-3xl font-black text-white italic">{educationData[0].gpa}</div>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="text-[9px] font-black text-yellow-500 animate-pulse">OPTIMIZED</div>
              </div>
            </div>
          </CyberScreen>
        </StackLayer>
        ...
        {/* CLEARANCES */}
        <StackLayer index={projects.length + 3}>
          <CyberScreen>
            <h2 className="text-3xl md:text-5xl font-black text-white italic uppercase mb-8">Clearances</h2>
              ...
              {certifications.map((cert, i) => (
                <div key={i} className="flex flex-col md:flex-row justify-between items-center p-5 bg-white/5 border border-white/10 hover:border-yellow-500/50 transition-all gap-4 group">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="text-white font-black uppercase text-base">{cert.title}</div>
                      <div className="text-yellow-500/60 font-mono text-[9px] uppercase">{cert.organization}</div>
                    </div>
                  </div>
            ...
            <div className="mt-8 pt-6 border-t border-white/5 flex justify-between items-center">
              <span className="text-[8px] font-mono uppercase text-gray-500 tracking-[0.4em]">©2027 ALOK.PRIME</span>
              <span className="text-[8px] font-mono uppercase text-yellow-500/40">Status: End of Payload</span>
            </div>
          </CyberScreen>
        </StackLayer>
      </div>
